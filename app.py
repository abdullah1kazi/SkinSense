...
def documentation_page():
    ...
    st.markdown(
        ...
        <div style="margin-top: 10px;">
            <span class="stat-text">Multi-Class Classifier Accuracy: <span style="color: #178ccb;">{multi_accuracy:.0%}</span></span>
        </div>
        """,
        unsafe_allow_html=True
    )
...
def show_abdullah_kazi():
    ...
    try:
        logo_image = "SkinSense.png"
        logo_image_data = open(logo_image, "rb").read()
        logo_encoded = base64.b64encode(logo_image_data).decode()
...
def show_skinsense(benign_diagnoses, malignant_diagnoses, class_names):
    ...
    class ImageClassifier(torch.nn.Module):
        ...
        def __init__(self, model_path, config_path, dropout_prob=0.5):
            super().__init__()

            # Load the configuration for the model
            config = AutoConfig.from_pretrained(config_path)

            # Define the model architecture as per the original architecture
    ...
    def predict_image(image, model, class_names):
        ...
        return probability_binary, probabilities_multi
    ...
    if uploaded_image is not None:  # only execute if an image has been uploaded
        ...
        probability_binary, probabilities_multi = predict_image(image, model, transform)
